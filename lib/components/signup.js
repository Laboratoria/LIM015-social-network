...
import { showAndHidePassword } from './login.js';
import { formValidation } from './validation.js';
import { succesfulSignUpModal } from './modal.js';

const signUpWithFirebase = (container) => {
    ...
    addUser(email, inputs[2].value)
      .then((userCredential) => {
        ...
        user.sendEmailVerification()
          .then(() => {
            container.appendChild(succesfulSignUpModal(email));
          });
        console.log(user);
...
export const signUp = () => {
  ...
  signUpWithFirebase(container);
  formValidation(element);
  // container.appendChild(succesfulSignUpModal('algo@.com'));
  return container;
};
