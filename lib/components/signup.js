    ...
    addUser(emailInput.value, passwordInput.value)
      ...
      .catch((error) => {
        if (error.message) {
          ...
          container.querySelector('#loginRedirectModal').addEventListener('click', () => {
            redirectTo('login');
          });
        }
...
export const signUp = () => {
  ...
  element.innerHTML = `
    <form id="signUp"  class="signUpForm">
      ...
      <label class="formText question">¿Ya tienes una cuenta?<a><span id="loginRedirect"><b>Inicia sesión</b></span></a></label>
    </form>
  `;
